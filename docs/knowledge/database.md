...
**`service_intake` (24) — the read/write contract, and the 2000-character cap.** Accessed only through `booking_engine/db/service_intake_queries.py`; the routes are `GET /api/v1/voice/config/{shop_id}/intake` and `PUT /api/v1/voice/config/{shop_id}/intake/{service_id}` (under voice config, not WhatsApp — the phone agent reads the same rows).

- **`MAX_QUESTIONS_CHARS = 2000`, enforced on write, in `normalise()`.** The text is re-read into the agent's prompt on *every turn of every conversation that touches the service*, so an owner who pastes an essay pays for it on each one and the agent's own instructions drown in it. Raised from 500 for the webapp's builder, which composes several questions per service. Over the cap is **trimmed then truncated, never refused** — the webapp shows a live character counter against the same number, so a 2001-character request means a second client or a stale draft, and storing the first 2000 beats a 422 the owner cannot interpret. A UI-only counter would not be a cap.
