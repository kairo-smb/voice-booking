...
| `POST` | `/api/v1/messaging/tick` | `messaging_tick.py` | Manual trigger for `run_tick`, which the in-process scheduler runs hourly (`services/scheduler.py`); both take the same advisory lock, and a busy lock returns `{"skipped": "busy"}`. Polls every `pending_review` request's Twilio bundle status, purchases the number for any newly-approved bundle, marks any newly-rejected one, refreshes the health semaphore for every already-provisioned number, then — last, so its failure can never suppress the health refresh above it — runs the grace-period number-release sweep (`services/number_release.py::sweep`). Returns counts: `{"reviewed", "provisioned", "rejected", "errors", "health", "release"}`, where `release` is `{"scheduled", "cleared", "released", "errors"}`. One shop's failure doesn't abort the sweep; a release-sweep failure is caught, counted under `errors`, and never turns into a 500. |
| `POST` | `/api/v1/voice/numbers/release` | `voice_telephony.py` | Owner-initiated release: `{"shop_id", "reason"}` → calls `services/number_release.py::release_for_shop` directly, bypassing the 14-day grace period `sweep` applies to a lapsed plan (a salon asking to give up its number is not a lapsed-plan guess). `"released"` → 200 `{"data": {"status": "released"}}`; `"no_number"` → 404 (nothing provisioned for that shop); `"release_failed"` → 502 (Twilio refused — the local row is deliberately left in place so the next `sweep` tick retries it). |
...
- **Release sweep** (`services/number_release.py::sweep`) runs last, after health. For every provisioned number: no plan behind it and nothing scheduled yet → schedules a release 14 days out (`GRACE_DAYS`); no plan and still before that deadline → no-op (re-scheduling every tick would push the deadline forward forever and the number would never actually release); the plan came back before the deadline → clears the schedule; no plan and the deadline has passed → releases via `release_for_shop` (Twilio first, then the local `shop_telephony` row, so a number is never orphaned mid-release). Wrapped in its own try/except so a Twilio hiccup here is counted under the top-level `errors` and the `release.errors` sub-count, not raised — a failure in this stage must never mask the health refresh that already ran above it, nor 500 the whole tick.
