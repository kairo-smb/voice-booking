...
| Tool bearer | `Authorization: Bearer <VOICE_AGENT_TOOL_SECRET>` | OpenAI Realtime (tool calls + session events), and the marketing-engine booking agent (tool calls only) | [Voice Tools](voice-tools.md), and the `/mcp` mount |
